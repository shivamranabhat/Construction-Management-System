                ...
                <div class="border-bottom px-4 py-3">
                    <div class="row g-3">
                        ...
                        <div class="col-md-3">
                            ...
                            @error('purchase_number') <small class="text-danger">{{ $message }}</small> @enderror
                        </div>
                        <div class="col-md-3">
                            <label>Vendor</label>
                                    ...
                                    <tr>
                                        <td class="position-relative">
                                            <div class="input-group input-group-sm">
                                                ...
                                                <input
                                                    type="text"
                                                    wire:model.live="lines.{{ $i }}.item_name"
                                            ...
                                            @if(isset($showItemDropdown[$i]) && $showItemDropdown[$i])
                                                <div class="bg-white border rounded shadow-sm mt-1"
                                                    ...
                                                    @php
                                                        $query = $item_search[$i] ?? '';
                                                        $items = \App\Models\Item::where('company_id', auth()->user()->company_id ?? 1)
                                                    ...
                                                    @empty
                                                        <div class="p-3">
                                                            @if(strlen($query) > 0)
                                                                <p class="text-muted mb-2 border-bottom pb-2">No results found. Try a different keyword.</p>
                                                            @else
                                        ...
                                        <td>
                                            <select wire:model.live="lines.{{ $i }}.tax_id" class="form-select form-select-sm">
                                                <option value="">Select a Tax</option>
                                                @foreach($taxes as $t)
                                                    <option value="{{ $t->id }}">{{ $t->name }} ({{ $t->rate }}%)</option>
                                                @endforeach
                                            </select>
                                            @error('lines.' . $i . '.tax_id')
                                                <small class="text-danger d-block">{{ $message }}</small>
                                            @enderror
                                        </td>
